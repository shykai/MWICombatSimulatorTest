...
class SimResult {
    ...
    addTimeSeriesSnapshot(time, players) {
        ...
        players.forEach(player => {
            ...
            // 计算累计伤害
            let totalDamage = 0;
            if (this.attacks[player.hrid]) {
                Object.values(this.attacks[player.hrid]).forEach(targetAttacks => {
                    Object.values(targetAttacks).forEach(abilityAttacks => {
                        Object.entries(abilityAttacks).forEach(([damage, count]) => {
                            totalDamage += parseInt(damage) * count;
                        });
                    });
                });
            }
            playerData.totalDamage.push(totalDamage);
        });
    }
}
